...
def get_books_read_by_month(username):
    ...
    :param username: The username for which the books needs to be returned
    :return: dict of month wise books read
    """

    current_year = datetime.datetime.now().year
    books = Review.objects.filter(creator__username__contains=username, date_created__year=current_year)\
        .values('date_created__month').annotate(book_count=Count('book__title')) #Values=projekció, Annotate=group by
